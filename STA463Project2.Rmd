...
We have low VIF scores very close to 1, indicating that we do not have any issues with multicollinearity. We have a few high leverage points with leverages above 0.10. Observation 971 appears to be the point with very high Cook's ditance, DFFITS, and DFBETAS x1. We should definitely exclude observation 971 if we want to analyze a fitted model without outliers.
...
eliminated2<- subset(airbnb.mod, airbnb.mod$logrstandard > -3 & airbnb.mod$logrstandard < 3)

airbnb.lm.log.noOut2 <- lm(log(price) ~ log(cleanliness_rating) + log(dist) + private_room + shared_room, data=eliminated2)
summary(airbnb.lm.log.noOut2)
```
...
airbnb$rstandard <- rstandard(airbnb.lm.log)

airbnbNOoutliers2 <- airbnb[-c(353,1057,539,215,726,389,575,272,254,1099,159,8,100,134,971),]

ggplot(airbnbNOoutliers2, aes(x=room_type, y=price,group=room_type)) + 
  geom_boxplot()
tapply(airbnbNOoutliers2$price, airbnbNOoutliers2$room_type, summary)

Q <- quantile(airbnb$price, probs=c(.25, .75), na.rm = FALSE)
...
eliminated<- subset(airbnb, airbnb$price > (Q[1] - 1.5*iqr) & airbnb$price < (Q[2]+1.5*iqr))

ggplot(eliminated, aes(x=room_type, y=price,group=room_type)) + 
  geom_boxplot()
```
