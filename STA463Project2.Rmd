...
From this interval, we are 95% confident that the mean price for a private room will be between 391 to 305 Euros less compared to an entire apartment/house. We are also 95% confident that the mean price for a shared room will be between 684.66 to 97.22 Euros less compared to an entire apartment/house.
...
Based on our results, we can conclude that the increase in price for an airbnb on average is higher for an entire home/apartment as compared to shared rooms and private rooms. This means that an airbir of an entire home/apartment is more likely to have a higher price compared to an airbnb of a private room and shared room. For two airbnbs, one being a private or shared room and one being an entire home/apartment, with the same cleanliness rating and distance from city center, we can expect, on average, that the airbnb that's an entire house/apartment to cost more than the private or shared room. A possible limitation of this project is that there is some ambiguity about how the data was collected. We assume that all observations are independent, although it is not completely clear how it was collected. Since the dataset is large and the airbnbs were from different popular European cities, we believe that the airbnbs shouldn't affect one another and that it is safe to make this assumption.
...
```
After log transforming our predictors and y-value, we can compare with removing outliers and high leverage points. In the Residual vs Fitted plot, it seems that there are some outlier residuals, as well as in the Scale-Location plot, (one specifically is observation 971). It seems that after log transforming the original fitted model, there is no longer a fanning effect in our Residual vs Fitted and Scale-Location plots. Thus, the model form assumption is met, and the constant variance assumption is met. We can also assume independence because the price of one Airbnb listing in one city shouldn't affect the price of an Airbnb listing in another city. In the Normal Q-Q plot, it seems that observation 971 is an outlier, but overall, the normality assumption seems good. After the removal of outliers and high leverage points, our assumptions are still met.  

Furthermore, in our summary statistics, it appears that the removal of outliers affected the Adjusted R-Squared because it increased from 0.48 to 0.53. Also, the significance of log(cleanliness_rating) improved a lot because the p-value reduced from 0.124 to 0.0525. 
