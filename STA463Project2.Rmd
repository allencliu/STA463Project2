...
A possible limitation of this project is that there is some ambiguity about how the data was collected. We assume that all observations are independent, although it is not completely clear how it was collected. Our data set is large, satisfying requirements for a large sample size. We believe that one Airbnb listing doesn't affect other listings, and it is safe to assume independence among observations, however, this is not always the case since there may be inherent competition in the market (e.g. price wars), but for the sake of our model we'll assume independence.
