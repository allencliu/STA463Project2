...
In the original Airbnb data set, the column X is merely an index column, and thus should be removed before performing any analyses. We will also remove any rows with N/A values if they are present. Since `room_type` is a categorical variable with 2 factors, we will encode it as a binary variable, where Entire home/apt is 0 and Private room is 1.

```{r, echo=FALSE}
airbnb <- airbnb[c("price", "cleanliness_rating", "room_type", "dist")]
airbnb <- na.omit(airbnb)
airbnb$room_type <- as.factor (airbnb$room_type) 
airbnb$room_type <- as.numeric(airbnb$room_type) - 1
head(airbnb)
```
...
We will examine the distribution of Airbnb prices grouped by Airbnb room types through boxplots.
