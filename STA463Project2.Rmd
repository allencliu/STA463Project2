...
We will evaluate a confidence interval on the predictors for the reduced model:

```{r,echo=FALSE}
confint(airbnbNO.lm.log.red)
```

From this interval, we are 95% confident that the geometric mean Airbnb price for a private room is expected to be a factor of between exp(-0.6771955) = 0.5080398 and 0.5523774, compared to the price of an Airbnb in an entire apartment/house. This means that, with 95% confidence, the geometric mean Airbnb price for a private room is expected to be between 50.8% and 55.2% of the price of an Airbnb in an entire apartment/house. We are also 95% confident that the geometric mean Airbnb price for a shared room is expected to be a factor of between exp(-1.1636835) = 0.3123336 and exp(-0.4724226) = 0.62349. This means that, with 95% confidence, the geometric mean Airbnb price for a shared room is expected to be between 31.2% and 62.3% of the price of an Airbnb in an entire apartment/house.
